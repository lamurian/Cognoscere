...
export default function (pi: ExtensionAPI) {
	pi.registerCommand("commit", {
		...
		handler: async (_args: string, ctx: ExtensionCommandContext) => {
			...
			const { stdout: patchOutput } = await pi.exec("git", [
				"diff",
				"--cached",
				"--patch",
			]);
			const { stdout: statOutput } = await pi.exec("git", [
				"diff",
				"--cached",
				"--stat",
			]);
			...
			if (!diffContent) {
				...
				const { stdout: fallback } = await pi.exec("git", [
					"diff",
					"--cached",
					"--name-status",
				]);
			...
			const diffSection = diffContent.length > MAX_DIFF_SIZE
