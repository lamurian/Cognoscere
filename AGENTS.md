...
4. Answer concisely. Cite sources using `[@citekey]` from `@ref.bib`.
5. Create with `create_para_doc` or `batch_create_para_docs` following the [knowledge skill's `Document Creation Standards`](.agents/skills/knowledge/SKILL.md#document-creation-standards).
6. Always run `list_para_tags` first. Pick existing tags. Only create new ones when none fit.
7. Standard frontmatter: title, description (max 200 chars), author (pi), editor (lam), date (ISO 8601), tags, source (optional) — auto-generated by the creation tools.
...
> **Document creation authority:** All PARA documents must follow the specifications in the knowledge skill's `Document Creation Standards` (@.agents/skills/knowledge/SKILL.md section `Document Creation Standards`). This is the single reference for atomic principle, language, citation style, cross-referencing, and classification.
